...
//gic_cpu_map 数组的每个元素表示一个 CPU，数组的索引即为 CPU 的逻辑编号。
//系统软件使用的是 CPU 逻辑 ID，通过 smp_processor_id 可以获取本 CPU 的逻辑ID
//gic_cpu_map这个全部lookup table就是用CPU 逻辑ID作为索引，去寻找其cpu mask，后续通过cpu mask值来控制中断是否送达该CPU
//在gic_init_bases函数中，我们将该lookup table中的值都初始化为0xff，也就是说不进行mask，送达所有的CPU
...
//如果 irq_data->common->handler_data 不为空说明是一个 cacading interrupt
//
...
static void gic_handle_cascade_irq(struct irq_desc *desc)
{
	...
	//读取secondary gic 的ACK寄存器
	status = readl_relaxed(gic_data_cpu_base(chip_data) + GIC_CPU_INTACK);

	//从 secondary gic 获取硬件中断号
	gic_irq = (status & GICC_IAR_INT_ID_MASK);
	if (gic_irq == GICC_INT_SPURIOUS)
		goto out;

	//将 seconfary gic 发生中断的 hwirq 转换为 irq
...
void __init gic_cascade_irq(unsigned int gic_nr, unsigned int irq)
{
	...
	//handle_data 指的是 gic_chip_data 的地址
	irq_set_chained_handler_and_data(irq, gic_handle_cascade_irq,
					 &gic_data[gic_nr]);
}

//从 DIST_TARGET 中获取 mask 值
...
static void gic_dist_init(struct gic_chip_data *gic)
{
	...
	//初始化GIC其他的寄存器，设置为低电平触发、禁止所有中断、优先级为 0xa0
...
static int gic_cpu_init(struct gic_chip_data *gic)
{
	...
	if (gic == &gic_data[0]) {
		...
		for (i = 0; i < NR_GIC_CPU_IF; i++)
			//清除lookup table中其他entry中本cpu mask的那个bit ？？？？？
			if (i != cpu)
				gic_cpu_map[i] &= ~cpu_mask;
	}

	//禁止所有的PPI中断，使能所有的SGI中断
	gic_cpu_config(dist_base, NULL);

	//设置中断优先级，只有优先级大于此值的中断可以被送达CPU
	//前面每个中断设置的默认优先级为 0xa0，这里设置为 0xf0，即不做限制，都可以通过
...
{
	//这段代码前面主要是分配两个per cpu的内存。这些内存在系统进入sleep状态的时候保存PPI的寄存器状态信息，在resume的时候，写回寄存器
				...
				irq_hw_number_t hw)
{
	...
	if (hw < 32) {
		//分配percpu相关内存，设置percpu一些flag
		irq_set_percpu_devid(irq);
		//设置irq的chip和handler、hwirq
		//其中 handler_irq 为 handle_percpu_devid_irq
			  ...
			  struct fwnode_handle *handle)
{
	...
	//配置 NON—Banked 的情况，大概意思就是要重新给每个寄存器地址加上一个偏移量
	...
	if (handle) {		/* DT/ACPI */
		 ...
		 */
		//对于 root GIC 来说，跳过 SGI，所以 -16
		if (gic == &gic_data[0] && (irq_start & 31) > 0) {
			hwirq_base = 16;
			if (irq_start != -1)
				irq_start = (irq_start & ~31) + 16;
		//对于 secondary GIC 来说，还要跳过 PPI，所以总共减去 -32
		} else {
			hwirq_base = 32;
		}
		//减去前面预留的中断号
		...
		//从 irq_start 开始分配 gic_irqs 个中断描述符，返回开始值，这里应该就是 irq_start 
				   ...
				   struct fwnode_handle *handle)
{
	...
	if (gic == &gic_data[0]) {
		...
		//设置 handle_arch_irq，这在进入中断的 entry 中会被调用
...
gic_of_init(struct device_node *node, struct device_node *parent)
{
	...
	//如果映射成功，of_iomap() 函数返回一个指向内核虚拟地址空间的指针，该地址对应于设备树节点中描述的内存资源
...
/*在一些 ARM 系统中，ACPI 也被用于描述硬件设备，包括 GIC。
在这些系统中，ACPI 表提供了关于 GIC 的配置信息，如 GIC 的基地址、中断号范围等。
操作系统（如 Linux 内核）需要解析 ACPI 表，根据提供的信息来初始化和配置 GIC。*/
