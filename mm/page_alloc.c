 ...
 */
//从伙伴系统中获取内存，若所需要的内存块不能满足，那么可以从大内存块中切内存
//返回内存块第一个页面的page结构的地址
static inline
struct page *rmqueue(struct zone *preferred_zone,  //首选的zone
			struct zone *zone, unsigned int order,  //当前遍历的zone，2^order
			gfp_t gfp_flags, unsigned int alloc_flags, //分配掩码，内部分配器的标志
			int migratetype)  //
{
	unsigned long flags;
	struct page *page;
	//如果只分配一页，那么直接从per_cpu_pages链表中分配
	if (likely(order == 0)) {
	...
	do {
		...
		//分配不成功，调用 __fallback 类型的分配函数再分配
		if (!page)
			page = __rmqueue(zone, order, migratetype, alloc_flags);
			//如果页面分配出来了，还要检查页面是否合格
			 ...
			 long free_pages)
{
	...
	for (o = order; o < MAX_ORDER; o++) {
		...
		//	MIGRATE_UNMOVABLE,
	// MIGRATE_MOVABLE,
	// MIGRATE_RECLAIMABLE,  检查这几个类型中的页面是否有满足分配的
...
retry:
	...
	z = ac->preferred_zoneref;   //首选的zone
...
try_this_zone:
	 ...
	 */
	//遍历了所有的zone之后还是没有分配出合适的内存，可能产生了外部碎片化，重新尝试
	if (no_fallback) {
		alloc_flags &= ~ALLOC_NOFRAGMENT;
		goto retry;
	}
 ...
 */
//gfp_mask 表示分配标志，order表示连续分配页面的数量，实际为2^次方，preferred_nid 表示优先分配的内存节点
struct page *
							...
							nodemask_t *nodemask)
{
	...
	alloc_mask = gfp_mask;
	//计算相关信息并将它们保存到分配上下文里面, 初始化 alloc_context，并为接下来的快速内存分配设置相关 gfp
	...
	//尝试从伙伴系统的空闲链表中分配内存，若分配成功，直接返回内存块的第一个页面的page数据结构
	//分配不成功，则会进入慢速分配流程
